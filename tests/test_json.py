...
schemas = [(path, name, data) for path, name, _, data in walk_json_data(top=absolute_source_schema_dir) if is_json_schema(data)]

with open(os.path.join(this_dir, 'schema', 'metaschema.json')) as fp:
    metaschema = Draft202012Validator(json.load(fp))
