...
OFDS offers two "containers" for publishing OFDS data in JSON format.

If your data is small enough to fit into memory or if you are publishing data via an API, you should use the [small files and API responses container](#small-files-and-api-responses-container). If your data is too large to fit into memory, you should use the [streaming container](#streaming-container).
...
## Small files and API responses container
...
Use the canonical network package schema URL to make sure that your software, documentation or other resources refer to the specific version of the schema with which they were tested. The canonical URL for version 0.4.0 is:

[https://standard.ofds.info/en/0\_\_4\_\_0/network-package-schema.json](https://standard.ofds.info/en/0__4__0/network-package-schema.json)
...
The streaming option is a [JSON Lines](https://jsonlines.org/) file in which each line is a valid OFDS network, as described by the [network object schema](network_schema.md).
